...
export default function SignupPage(props) {
    ...
    useEffect(() => {
        ...
        setSavedLoginArray(JSON.parse(localStorage.getItem('QC-TB-SavedLoginArray')) || []);
        setLoading('');
    ...
    let saveLoginInfo = (user) => {
        let temp = [...savedLoginArray];
        temp.push({ email: user.email, avatar: user.avatar, uid: user.key })
    ...
    const sendVerificationEmail = (user) => {
        ...
        emailjs.send("service_ur2epnr", "template_jtj9yhy", {
            ...
            .then(() => {
                ...
                localStorage.setItem('QC-Techbook-AccountVerifyLinkData', JSON.stringify({ date: new Date().toLocaleDateString(), email: user.email, key: key }))
            }).catch(error => {
                console.error(error);
                props.updateNotification({ type: 'red', msg: 'Something went wrong! Please try again.' });
            });
